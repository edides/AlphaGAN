...
# Decoder only contains ResNet50 and output_stride = 8 
#''''''''''''''''''''''''''''''''''''#
...
class ResNet(nn.Module):
	...
	def reslayer(self, num_of_blocks, channel, stride=1, dilation=1, expension=4):
		...
		layers.append(Res50_BottleNeck(self.inplanes, channel, stride=stride, dilation=dilation, ds=True))
		self.inplanes = channel * expension

		for i in range(1, num_of_blocks):
			layers.append(Res50_BottleNeck(self.inplanes, channel, stride=1, dilation=dilation, ds=False))
	...
	def forward(self, x):
		...
		#print('*'*10, skip_connection3.size())
		out = self.conv_3(out)
		out = self.conv_4(out)
		out = self.conv_5(out)
...
class Res50_BottleNeck(nn.Module):
	...
	def __init__(self, in_channel, num_channels, stride, dilation=1, ds=False):
...
class DeepLabv3_plus(nn.Module):
	...
	def __init__(self, input_channel=4, initial_mode=True, pretrained=False, _print=True):
		if _print:
		...
		self.aspp1 = ASPP_module(2048, 256, 1, initial_mode=True)
		self.aspp2 = ASPP_module(2048, 256, 6, initial_mode=True)
		self.aspp3 = ASPP_module(2048, 256, 12, initial_mode=True)
		self.aspp4 = ASPP_module(2048, 256, 18, initial_mode=True)

		self.relu = nn.ReLU()
		
		self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d(41), # apply aap and get 41x41 output
...
class Generator(nn.Module):
	...
	'''
	def __init__(self, in_channel, os, initial_mode=True, pretrained=False):
		super(Generator, self).__init__()
		self.Encoder = Encoder(in_channel, os, initial_mode, pretrained, _print=True)
		...
		self.conv_3x3_block2 = nn.Sequential(nn.Conv2d(96, 64, kernel_size=3, padding=1, bias=False),
											nn.BatchNorm2d(64),
		...
		self.conv_3x3_block3 = nn.Sequential(nn.Conv2d(36, 32, kernel_size=3, padding=1, bias=False),
											nn.BatchNorm2d(32),
											nn.ReLU(inplace=True),
	...
	## I haven't implement the comp_loss in this section
	def forward(self, in_data, batch_alphasT, batch_trimapsT, gt):
		x, skip_connection1, skip_connection2, skip_connection3 = self.Encoder(in_data)
		#print('******************x5', x.size())
		x = F.upsample(x, size=(int(math.ceil(in_data.size()[-2]/4) + 1), 
			int(math.ceil(in_data.size()[-1]/4) + 1)), mode='bilinear', align_corners=True)
		...
		x = F.upsample(x, size=(int(math.ceil(in_data.size()[-2]/2)), 
			int(math.ceil(in_data.size()[-1]/2))), mode='bilinear', align_corners=True)
		...
		x = F.upsample(x, size=(int(math.ceil(in_data.size()[-2])), 
			int(math.ceil(in_data.size()[-1]))), mode='bilinear', align_corners=True)
		...
		pred_mattes = x #([1, 1, 320, 320])
		alpha_diff = torch.sqrt((pred_mattes - batch_alphasT.float())**2 +1e-12)

		# A torch.ByteTensor containing a 1 at each location where comparison is true
		cond = torch.eq(batch_trimapsT, 128) # where the unknown region is
		#print('#'*30, cond)
		cond = cond.type(torch.cuda.FloatTensor)
		wl =  cond * Variable(torch.ones([batch_trimapsT.shape[0], img_rows, img_cols]).cuda()) + ((1-cond) *  Variable(torch.zeros([batch_trimapsT.shape[0], img_rows, img_cols]).cuda())) 
		unknown_region_size = wl.sum()

		alpha_loss = (alpha_diff * wl).sum()/unknown_region_size

		############################################################
		# Following is added for composition loss
		############################################################

		
		pred_rgb = gt.float() * (pred_mattes) + (torch.ones_like(pred_mattes) - pred_mattes) * gt.float()
		c_diff = torch.sqrt((pred_rgb - gt.float())**2 +1e-12)/255
		c_loss = (c_diff * wl).sum()/unknown_region_size
		...
		if math.isnan(alpha_loss.item()) is not True:
			combine_loss = c_loss + alpha_loss
		else:
			combine_loss = c_loss
		print("combine losssssssssssssssss", combine_loss)
		return combine_loss, pred_rgb, pred_mattes
...
def Encoder(input_channel, os, initial_mode, pretrained, _print=True):
	model = DeepLabv3_plus(input_channel=4, pretrained=pretrained, _print=True)
	# self, input_channel=4, initial_mode=True, pretrained=False, _print=True
	return model
